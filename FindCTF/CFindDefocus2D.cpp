...
{	m_gfSpect = gfSpect;
	...
	//-----------------------------------------------------------
	for(int i=0; i<10; i++)
	{	float fScale = 1.0f - i * 0.02;
		mRefineDfMean(fDfMeanRange * fScale);
		if(m_fCCMax > 0.001f) break;
	}
	//----------------------------------
...
float CFindDefocus2D::mRefineDfMean(float fDfRange)
{
	...
	float fDfStep = (fDfMeanN - fDfMean0) / (iNumSteps - 1);
	//-----------------
	float fDfMeanMax = 0.0f, fCCMax = (float)-1e20;
	for(int i=0; i<iNumSteps; i++)
	{	m_fDfMean = fDfMean0 + i * fDfStep;
...
float CFindDefocus2D::mRefinePhase(float fPhaseRange)
{
	...
	float fPhaseN = fmin(fPhase0 + fPhaseRange, 180.0f);
